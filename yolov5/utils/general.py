...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45):
    # Placeholder simplified version of NMS (for real use, consider using torchvision.ops.nms)
    boxes = []
    for pred in prediction:
        pred = pred[pred[:, 4] > conf_thres]  # filter by confidence
        if not pred.size(0):
            continue
        scores = pred[:, 4]
        keep = torch.ops.torchvision.nms(pred[:, :4], scores, iou_thres)
        boxes.append(pred[keep])
    return boxes
